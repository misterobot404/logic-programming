...
task_win_eh(_Win,e_Create(_),0):- dlg_main_window_Create(_Win), win_Destroy(_Win),!,
...
%BEGIN main_window, CreateParms, 21:13:11-23.2.2019, Code automatically updated!
...
domains
  startNumber,endNumber,currentValue = integer
...
predicates
  ...
  check_prime_number(startNumber,currentValue)
...
%MARK main_window, new variables
	...
	dialog_CreateModal(Parent,dlg_main_window_ResID,"",
  		[
%BEGIN main_window, ControlList, 21:13:11-23.2.2019, Code automatically updated!
...
%BEGIN main_window, Update controls, 21:13:11-23.2.2019, Code automatically updated!
...
%BEGIN main_window, idc_ok _CtlInfo
  ...
  dlg_main_window_eh(_Win,e_Control(idc_ok,_CtrlType,_CtrlWin,_CtlInfo),0):-
  dlg_Error("Ошибка валидации","Слишком большое число. Выход за пределы диапазона!"),
  	!.
%END main_window, idc_ok _CtlInfo

%BEGIN main_window, idc_help _CtlInfo
  dlg_main_window_eh(_Win,e_Control(idc_help,_CtrlType,_CtrlWin,_CtlInfo),0):- dlg_Note("Задание","Написать программу для построения ряда простых чисел. Пределы ряда задаются пользователем, вывод результата осуществить в отдельное окно. Реализовать систему сообщений об ошибках "),
  	!.
...
prime_numbers(StartNumber,EndNumber,_Win):-
StartNumber>EndNumber,
dlg_Note("Сообщение","Поиск ряда простых чисел закончен"),
	!.

prime_numbers(StartNumber,EndNumber,_Win):-
check_prime_number(StartNumber, 2),
EditHandle = win_GetCtlHandle(_Win,idc_edit),
str_int(StartString, StartNumber),
...
check_prime_number(Number,CurrentValue):-
Number - 1 <= CurrentValue,
	!.

check_prime_number(Number,CurrentValue):-
Number mod CurrentValue <> 0,
CurrentValuePlusOne = CurrentValue + 1,
check_prime_number(Number, CurrentValuePlusOne).
