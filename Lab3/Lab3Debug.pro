...
predicates
/*  
Вставка элемента после каждого вхождения
    1 - вставляемый элемент
    2 - элемент после которого происходит вставка
    3 - исходный список
    4 - измененный список 
*/
...
/*
Объединение трех списков в один в указанном  порядке
    1 - номер списка, который будет добавлен первым
    2 - номер списка, который будет добавлен вторым
    3 - первый список
    4 - второй список
    5 - третий список
    6 - получившийся список
*/
task3_entry()
...
/*
Сортировка по убыванию метод вставки
    1 - исходный список
...
clauses
%Task 1
insert(_,_,[],[]):- write("Список очищен, возвращаемся по точкам отката и добавляем элементы"), nl.
insert(X,Head,[Head|Z],[Head,X|T]):- write("На обратном пути добавим ", Head, " и ", X), nl, insert(X,Head,Z,T), write("К текущему списку: ",T, " добавим ", Head, " и ", X), nl,!.
insert(X,Y,[Head|Z],[Head|N]):- write("На обратном пути добавим ", Head), nl, insert(X,Y,Z,N), write("К текущему списку: ",N, " добавим ", Head), nl.
%Task 2 
revers(X,Y):- revers([],X,Y).  
revers(Y,[],Y):- write("Дно рекурсии. Возвращем списки"),nl.  
revers(X1,[Z|X2],Y):- write("От списка X2: ", X2, " отнимаем ", Z, " и прибавляем к X1: ", X1), nl, revers([Z|X1],X2,Y), write("К списку X2: ", X2, " прибавляем ", Z, " и отнимаем от X1: ", X1), nl.
%Task 3
task3_entry():- 
%num
write("Списки:"), nl,
