...
clauses
  ...
  string_to_list("",[]):- !.
  string_to_list(String,[Symbol|Tail]):- count_symbol_before_separator(String, ",", CountSymbol, TextEnd), CountSymbol <> 0, TextEnd = 0, !, frontstr(CountSymbol, String, Symbol, StringTailTemp), frontstr(1, StringTailTemp, _, StringTail), string_to_list(StringTail, Tail).
  %string_to_list(String,[Symbol|Tail]):- count_symbol_before_separator(String, ",", CountSymbol, TextEnd), CountSymbol <> 0, TextEnd = 1, frontstr(CountSymbol, String, Symbol, Temp), Tail = [].
  
  count_symbol_before_separator(String, Separator, Count, TextEnd):- frontstr(1, String,Symbol,StringTail), Symbol <> Separator, StringTail <> "", count_symbol_before_separator(StringTail, Separator, CountPlusOne, TextEnd), Count = CountPlusOne + 1, !.
  count_symbol_before_separator("", _, Count, TextEnd):- Count = 0, TextEnd = 1, dlg_Node("Hello","H"), !.
  count_symbol_before_separator(_, _, Count, TextEnd):- Count = 0, TextEnd = 0, !.
  %count_symbol_before_separator(String, Separator, Count, TextEnd):- frontstr(1, String, StringHead, StringTail), StringTail = "", Count = 0, TextEnd = 1, !.
...
%BEGIN insert_el, CreateParms, 15:34:02-8.4.2019, Code automatically updated!
...
%MARK insert_el, new variables
	...
	dialog_CreateModal(Parent,dlg_insert_el_ResID,"",
  		[
%BEGIN insert_el, ControlList, 15:34:02-8.4.2019, Code automatically updated!
...
%BEGIN insert_el, Update controls, 15:34:02-8.4.2019, Code automatically updated!
...
%BEGIN revers, CreateParms, 10:29:16-7.4.2019, Code automatically updated!
...
%MARK revers, new variables
	...
	dialog_CreateModal(Parent,dlg_revers_ResID,"",
  		[
%BEGIN revers, ControlList, 10:29:16-7.4.2019, Code automatically updated!
...
%BEGIN revers, Update controls, 10:29:16-7.4.2019, Code automatically updated!
...
%BEGIN revers, idc_ok_num _CtlInfo
  dlg_revers_eh(_Win,e_Control(idc_ok_num,_CtrlType,_CtrlWin,_CtlInfo),0):-!,
  LBox_Handle = win_GetCtlHandle(_Win,id_edit),
  
  NumArray = [-5,4,0,-15,4,0,1,-5],
  revers(NumArray,NumArrayRes),
  num_list_to_string(NumArrayRes,StringRes),
  
  lbox_Clear(LBox_Handle),
  lbox_Add(LBox_Handle, StringRes),
	!.
%END revers, idc_ok_num _CtlInfo
...
%BEGIN ОБЪЕДИНЕНИЕ ТРЁХ СПИСКОВ В ОДИН В УКАЗАННОМ ПОРЯДКЕ, CreateParms, 11:55:21-7.4.2019, Code automatically updated!
...
%MARK ОБЪЕДИНЕНИЕ ТРЁХ СПИСКОВ В ОДИН В УКАЗАННОМ ПОРЯДКЕ, new variables
	...
	dialog_CreateModal(Parent,dlg_объединение_трёх_списков_в_один_в_указанном_порядке_ResID,"",
  		[
%BEGIN ОБЪЕДИНЕНИЕ ТРЁХ СПИСКОВ В ОДИН В УКАЗАННОМ ПОРЯДКЕ, ControlList, 11:55:21-7.4.2019, Code automatically updated!
		df(idc_lbox_res,listbox([],[0]),nopr),
		df(idc_edit_num_first,editstr("",[]),nopr),
		df(idc_edit_num_second,editstr("",[]),nopr),
...
%BEGIN ОБЪЕДИНЕНИЕ ТРЁХ СПИСКОВ В ОДИН В УКАЗАННОМ ПОРЯДКЕ, Update controls, 11:55:21-7.4.2019, Code automatically updated!
	dialog_VLGetListBox(idc_lbox_res,_VALLIST,_IDC_LBOX_RES_ITEMLIST,_IDC_LBOX_RES_SELECT),
	_IDC_EDIT_NUM_FIRST_VALUE = dialog_VLGetstr(idc_edit_num_first,_VALLIST),
	_IDC_EDIT_NUM_SECOND_VALUE = dialog_VLGetstr(idc_edit_num_second,_VALLIST),
...
%BEGIN ОБЪЕДИНЕНИЕ ТРЁХ СПИСКОВ В ОДИН В УКАЗАННОМ ПОРЯДКЕ, idc_ok_sym _CtlInfo
  ...
  append(First, Second, SymArray1, SymArray2, SymArray3, SymArray3Res),
...
%BEGIN СОРТИРОВКА ПО УБЫВАНИЮ МЕТОДОМ ВСТАВОК, CreateParms, 20:05:32-12.4.2019, Code automatically updated!
...
%MARK СОРТИРОВКА ПО УБЫВАНИЮ МЕТОДОМ ВСТАВОК, new variables
	...
	dialog_CreateModal(Parent,dlg_сортировка_по_убыванию_методом_вставок_ResID,"",
  		[
%BEGIN СОРТИРОВКА ПО УБЫВАНИЮ МЕТОДОМ ВСТАВОК, ControlList, 20:05:32-12.4.2019, Code automatically updated!
		df(id_edit,listbox([],[0]),nopr),
...
%BEGIN СОРТИРОВКА ПО УБЫВАНИЮ МЕТОДОМ ВСТАВОК, Update controls, 20:05:32-12.4.2019, Code automatically updated!
	dialog_VLGetListBox(id_edit,_VALLIST,_ID_EDIT_ITEMLIST,_ID_EDIT_SELECT),
